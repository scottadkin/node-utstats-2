...
import { plural, ignore0, getTeamColor } from '../../../../api/generic.mjs';
...
function renderTeam(teamId, playerData, matchId, single){
    ...
    for(let i = 0; i < playerData.length; i++){
        ...
        if(ctf.best_single_seal > 0){
            sealString = `${ctf.best_single_seal} ${plural(ctf.best_single_seal, "Kill")}`;
        }

        let bestSealString = "";

        if(ctf.flag_seal_best > 0){
            bestSealString = `${ctf.flag_seal_best} ${plural(ctf.flag_seal_best, "Kill")}`;
        }
        ...
        data.push({
            ...
            "flag_seal_pass": {"value": ctf.flag_seal_pass, "displayValue": ignore0(ctf.flag_seal_pass)},
            "flag_seal_fail": {"value": ctf.flag_seal_fail, "displayValue": ignore0(ctf.flag_seal_fail)},
    ...
    if(!single){
        ...
        data.push({
            ...
            "flag_seal_pass": {"value": ignore0(totals.flag_seal_pass)},
            "flag_seal_fail": {"value": ignore0(totals.flag_seal_fail)},
            "best_single_seal": {"value": `${totals.best_single_seal} ${plural(totals.best_single_seal, "Kill")}`},
            "flag_seal_best": {"value": `${totals.flag_seal_best} ${plural(totals.flag_seal_best, "Kill")}`},
        });
    }
...
export default function MatchCTFSummarySeals({playerData, matchId, single}){
