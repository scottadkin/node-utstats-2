...
async function calcBasicTotals(gametypeId, mapId){
    ...
    SUM(assault_caps) as assault_caps, SUM(amp_kills) as amp_kills, 
...
export async function calculateMapTotals(gametypeId, mapId){
    ...
    const vars = [
        ...
        g.best_spawn_kill_spree, g.assault_objectives,
...
async function insertMapTotals(gametypeId, mapId, data){
    ...
    const vars = [
        ...
        d.best_spawn_kill_spree, d.assault_objectives,
...
export async function recalculateTotals(gametypeId, mapId){
    ...
    for(let i = 0; i < result.length; i++){
        ...
        if(totalMatches === 0){
            ...
            if(gametypeId === 0 && mapId === 0) await deleteMap(mapId);
            continue;
