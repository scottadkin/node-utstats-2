...
const VALID_TYPES = ["gametype", "map", "allTime"];
...
export async function deletePlayerTypeTotals(type, targetId, playerIds, weaponIds){
    ...
    const index = VALID_TYPES.indexOf(type);
...
const WEAPON_TOTALS_EMPTY_OBJECT = {
    "total_matches": 0,
    "kills": 0,
    "max_kills": 0,
    "best_kills_spree": 0,
...
async function getWeaponTotalsFromMatchTable(weaponIds){
    ...
    MAX(best_kills) as best_kills_spree,
    ...
    FROM nstats_player_weapon_match WHERE weapon_id IN(?) GROUP BY weapon_id, gametype_id, map_id`;
    ...
    const higherBetterKeys = [
        ...
        "max_suicides",
        "max_team_kills",
        "best_team_kills_spree",
    ];
    
    const data = {};

    //weaponId => gametypeId => mapId

    for(let i = 0; i < result.length; i++){

        const r = result[i];

        if(data[r.weapon_id] === undefined){
            data[r.weapon_id] = {};
        }

        if(data[r.weapon_id][r.gametype_id] === undefined){
            data[r.weapon_id][r.gametype_id] = {};      
        }

        if(data[r.weapon_id][0] === undefined){
            data[r.weapon_id][0] = {};      
        }

        //all time totals
        if(data[r.weapon_id][0][0] === undefined){
            data[r.weapon_id][0][0] = {...WEAPON_TOTALS_EMPTY_OBJECT};
        }

        //map all time totals
        if(data[r.weapon_id][0][r.map_id] === undefined){
            data[r.weapon_id][0][r.map_id] = {...WEAPON_TOTALS_EMPTY_OBJECT};
        }

        //gametype all time totals
        if(data[r.weapon_id][r.gametype_id][0] === undefined){
            data[r.weapon_id][r.gametype_id][0] = {...WEAPON_TOTALS_EMPTY_OBJECT};
        }

        //gametype & map all time totals
        if(data[r.weapon_id][r.gametype_id][r.map_id] === undefined){
            data[r.weapon_id][r.gametype_id][r.map_id] = {...WEAPON_TOTALS_EMPTY_OBJECT};
        }


        const allTime = data[r.weapon_id][0][0];
        const mapTotals = data[r.weapon_id][0][r.map_id];
        const gametypeTotals = data[r.weapon_id][r.gametype_id][0];
        const comboTotals = data[r.weapon_id][r.gametype_id][r.map_id];
     
        const toUpdate = [allTime, mapTotals, gametypeTotals, comboTotals];

        for(let x = 0; x < toUpdate.length; x++){

            for(let k = 0; k < addKeys.length; k++){

                toUpdate[x][addKeys[k]] += parseInt(r[addKeys[k]]);
            }

            for(let k = 0; k < higherBetterKeys.length; k++){

                if(r[higherBetterKeys[k]] > toUpdate[x][higherBetterKeys[k]]){
                    toUpdate[x][higherBetterKeys[k]] = r[higherBetterKeys[k]];
                }
            }

            let accuracy = 0;

            if(toUpdate[x].hits > 0 && toUpdate[x].shots > 0){
                accuracy = toUpdate[x].hits / toUpdate[x].shots * 100;
            }

            toUpdate[x].accuracy = accuracy;
        }
    }
...
async function deleteWeaponTotals(id){
    ...
    const query = `DELETE FROM nstats_weapons_totals WHERE weapon_id=?`;

    return await simpleQuery(query, [id]);
}


async function deleteMultipleWeaponTotals(ids){
...
async function recalculateWeaponTotals(weaponIds){
    ...
    //deleteTotals(weaponIds, gametypeId, mapId)
    if(weaponIds.length === 0) return;

    const totals = await getWeaponTotalsFromMatchTable(weaponIds);

    //console.log(totals[Object.keys(totals)[0]][0][0]);

    await deleteMultipleWeaponTotals(weaponIds);

    const data = [];

    for(const [weaponId, gametypeData] of Object.entries(totals)){

        for(const [gametypeId, mapData] of Object.entries(gametypeData)){

            for(const [mapId, d] of Object.entries(mapData)){

                d.weapon_id = weaponId;
...
async function recalculatePlayerBest(type, id){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        if(best[d.player_id] === undefined){
            best[d.player_id] = {};
        }

        if(best[d.player_id][d.weapon_id] === undefined){

            best[d.player_id][d.weapon_id] = {...d};

            if(type === "gametype"){
                best[d.player_id][d.weapon_id].map_id = 0;
                best[d.player_id][d.weapon_id].gametype_id = id;
            }else if(type === "map"){
    ...
    for(const playerData of Object.values(best)){
    ...
    await deletePlayerBest(type, id);
...
async function deletePlayerTotals(type, id){
...
async function recalculatePlayerTotals(type, id){

    type = type.toLowerCase();

    if(VALID_TYPES.indexOf(type) === -1) throw new Error(`${type} is not a valid type for deletePlayerTotals`);

    let query =  `SELECT player_id,map_id,weapon_id,${PLAYER_TOTALS_MATCH_COLUMNS}
    FROM nstats_player_weapon_match
    WHERE gametype_id=?
    ...
    if(type === "map"){
        ...
        query =  `SELECT player_id,map_id,weapon_id,${PLAYER_TOTALS_MATCH_COLUMNS}
        FROM nstats_player_weapon_match
        WHERE map_id=?
    ...
    for(let i = 0; i < data.length; i++){
        ...
        const g = totals[d.player_id][d.weapon_id];
    ...
    await deletePlayerTotals(type, id);
...
export async function mergeGametypes(oldId, newId){
    ...
    await deletePlayerBest("gametype", oldId);
    await recalculatePlayerBest("gametype", newId);

    await deletePlayerTotals("gametype", oldId);
    await recalculatePlayerTotals("gametype", newId);
}
...
async function getPlayerUsedWeapons(playerId){
...
async function deleteSinglePlayerTotals(playerId, type){

    if(VALID_TYPES.indexOf(type) === -1) throw new Error(`Not a valid deleteSinglePlayerTotals type`);

    let query = `DELETE FROM nstats_player_weapon_totals WHERE player_id=?`;


    if(type === "allTime"){
        query += ` AND gametype=0 AND map_id=0`;
    }else if(type === "gametype"){

        query += ` AND map_id=0 AND gametype!=0`;
...
async function recalculateSinglePlayerTotals(playerId, type){

    if(VALID_TYPES.indexOf(type) === -1) throw new Error(`Not a valid recalculateSinglePlayerTotals type`);

    let query =  `SELECT weapon_id,${PLAYER_TOTALS_MATCH_COLUMNS}`;
    if(type === "gametype") query += `,gametype_id`;
    if(type === "map") query += `,map_id`;


    query += ` FROM nstats_player_weapon_match
    WHERE player_id=?
    GROUP BY weapon_id`;
    if(type === "gametype") query+= `,gametype_id`;
    if(type === "map") query+= `,map_id`;

    const data = await simpleQuery(query, [playerId]);
    ...
    await deleteSinglePlayerTotals(playerId, type);
    await bulkInsertPlayerTotals(data);
}

async function deleteSinglePlayerBest(playerId, type){

    if(VALID_TYPES.indexOf(type) === -1) throw new Error(`${type} is not a valid type for deleteSinglePlayerBest`);

    let query = `DELETE FROM nstats_player_weapon_best WHERE player_id=?`;

    if(type === "allTime"){
        query += ` AND gametype_id=0 AND map_id=0`;
    }else if(type === "gametype"){
        query += ` AND gametype_id!=0 AND map_id=0`;
    }else if(type === "map"){
        query += ` AND gametype_id=0 AND map_id!=0`;
...
async function recalculateSinglePlayerBest(playerId, type){


    if(VALID_TYPES.indexOf(type) === -1) throw new Error(`${type} is not a valid type for recalculateSinglePlayerBest`);

    let query = `SELECT weapon_id,map_id,
    ${PLAYER_BEST_MATCH_COLUMNS}
    FROM nstats_player_weapon_match
    WHERE player_id=? GROUP BY weapon_id,map_id`;

    if(type === "map"){

        query = `SELECT weapon_id,map_id,
        ${PLAYER_BEST_MATCH_COLUMNS}
        FROM nstats_player_weapon_match
        WHERE player_id=? GROUP BY weapon_id,map_id`;
        ...
        query = `SELECT weapon_id,gametype_id,
        ${PLAYER_BEST_MATCH_COLUMNS}
        FROM nstats_player_weapon_match
        WHERE player_id=? GROUP BY weapon_id,gametype_id`;
    }
    ...
    for(let i = 0; i < data.length; i++){
        ...
        if(best[d.weapon_id] === undefined){

            best[d.weapon_id] = {...d};

            if(type === "gametype"){
                best[d.weapon_id].map_id = 0;
            }else if(type === "map"){
                best[d.weapon_id].gametype_id = 0;
            }else if(type === "allTime"){
                best[d.weapon_id].gametype_id = 0;
                best[d.weapon_id].map_id = 0;
            }
    ...
    await deleteSinglePlayerBest(playerId, type);
    await bulkInsertPlayerBest(data);
...
export async function deletePlayerFromMatch(playerId, matchId, gametypeId, mapId){
    ...
    await recalculateSinglePlayerTotals(playerId, "allTime");
    await recalculateSinglePlayerTotals(playerId, "gametype");
    await recalculateSinglePlayerTotals(playerId, "map");
...
async function calculateWeaponTotals(weaponIds, gametypeId, mapId){
    ...
    SUM(kills) as kills,
    MAX(kills) as max_kills,
    MAX(best_kills) as kills_best_spree,
    SUM(deaths) as deaths,
    MAX(deaths) as max_deaths,
    SUM(shots) as shots,
    MAX(shots) as max_shots,
    ...
    for(let i = 0; i < result.length; i++){
        ...
        const shots = parseInt(r.shots);

        if(hits > 0 && shots > 0){
            r.accuracy = hits / shots * 100;
        }
...
async function bulkInsertTotals(data, matchesUsed, gametypeId, mapId){
    ...
    for(let i = 0; i < data.length; i++){
        ...
        if(matchesUsed !== undefined){

            matches = matchesUsed?.[d.weapon_id] ?? 0;

        }else{

            if(d.total_matches === undefined){
                throw new Error(`You must provide matches used as an argument or have a key of total_matches for each data element`);
            }
        ...
        if(gametypeId === undefined && d.gametype_id === undefined){
            throw new Error(`You must provide gametypeId as an argument or have a key of gametype_id for each data element`);
        }

        if(mapId === undefined && d.map_id === undefined){
            throw new Error(`You must provide mapId as an argument or have a key of map_id for each data element`);
        }

        insertVars.push([
            d.weapon_id, gametypeId ?? d.gametype_id, mapId ?? d.map_id, matches, d.kills, d.max_kills, d.kills_best_spree ?? d.best_kills_spree, d.deaths,
