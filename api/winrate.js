...
async function getPlayerLatestWinrateDate(playerIds, gametypeId, mapId){
    ...
    const query = `SELECT player,MAX(date) as latest_match FROM nstats_winrates WHERE player IN(?) AND gametype=? AND map=? GROUP BY player`;
...
async function getPlayerCurrent(playerIds, gametypeId, mapId){
    ...
    max_win_streak,max_draw_streak,max_lose_streak FROM nstats_winrates 
...
async function updateExistingPlayer(playerId, playerData, matchResult, matchDate, matchId, gametypeId, mapId){
    ...
    if(playerData.current_streak_type === matchResult){
        ...
        playerData.current_streak = matchResult;
    }
    ...
    const vars = [
        ...
        playerData.losses, winrate, playerData.current_streak_type, playerData.current_streak,
